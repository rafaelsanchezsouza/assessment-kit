...
// @assessment-kit/types models dates as ISO strings (Finding.effectiveDate, Evidence.documentDate).
...
export function getPool(): Pool {
  if (!pool) {
    pool = new Pool({
      connectionString: process.env.DATABASE_URL ?? 'postgres://akit:akit@localhost:5433/akit',
    });
  }
  return pool;
}
